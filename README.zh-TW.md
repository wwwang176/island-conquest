...
兩台中立**直升機**分別在兩端旗點附近生成。第一個登上的士兵為其隊伍佔領該直升機。
...
- **駕駛員**操控方向，無法開火。**乘客**可使用個人武器射擊，射界 160°（前方和後方有盲區）。
- 血量歸零時直升機被摧毀 — 所有乘員死亡。殘骸停留 10 秒後消失。
- 僅在接近地面（< 5m 高度）時可下機。
- AI 小隊會搭乘直升機前往遠方目標。
...
| 個性 | 瞄準技巧 | 基礎反應 | 近 / 遠 | 風險容忍 | 角色定位 |
|------|---------|---------|---------|---------|---------|
| **突擊兵 (Rusher)** | 0.7 | 150ms | 0.3 / 1.4 | 高 (0.75) | 激進尖兵 |
| **防禦兵 (Defender)** | 0.8 | 180ms | 0.6 / 0.9 | 低 (0.45) | 謹慎據守 |
| **側翼兵 (Flanker)** | 0.75 | 160ms | 0.4 / 1.2 | 中 (0.65) | 繞側突擊 |
| **支援兵 (Support)** | 0.7 | 190ms | 0.7 / 1.1 | 中 (0.55) | 壓制與封區 |
| **狙擊手 (Sniper)** | 0.9 | 150ms | 1.4 / 0.5 | 低 (0.50) | 遠程精準 |
| **隊長 (Captain)** | 0.8 | 170ms | 0.5 / 1.0 | 中 (0.60) | 小隊指揮、均衡 |
...
7. **撤退** — 退回友方旗點（小隊戰術）
8. **衝鋒** — 協同突擊（小隊戰術）
9. **壓制射擊** — 壓制失去視線的敵人（小隊戰術）
10. **交叉火力** — 展開至側翼位置（小隊戰術）
11. **投擲手榴彈** — 敵人在旗點附近聚集時
12. **近距離敵人**（< 20m）— 立即交戰
13. **未佔領旗點** — 前往佔領
14. **追查情報** — 前往可疑敵方位置
...
- **反應延遲**：發現新目標時的多因子延遲：
  - `基礎反應 × 距離因子 × 視線因子 × 角度因子 + 隨機(0–150ms)`
  - **距離因子**：在 `nearReaction`（0m）與 `farReaction`（60m+）之間線性插值。突擊兵近距離極速反應（0.3×）但遠距離較慢（1.4×）；狙擊手相反（1.4× / 0.5×）。
  - **視線因子**：全身可見為 1.0，僅露頭為 1.4（部分掩體）。
  - **角度因子**：`0.933 + 0.5 × (1 − dot)`，其中 `dot` 是面朝方向與敵人方向的餘弦值。敵人越靠近準星中心反應越快（~0.93×）；在視野邊緣則大幅減慢（~1.43×）。
...
src/
  ...
  entities/
    ...
    Grenade.js         手榴彈實體（物理 + 引信）
    Vehicle.js         載具基礎類別（HP、上下車、重生）
    Helicopter.js      直升機飛行模型 + 多乘客
  world/
    Island.js          程序化地形 + 植被
    CoverSystem.js     掩體點登錄
    FlagPoint.js       旗點佔領機制
    Fortification.js   防禦工事結構
    Noise.js           Simplex 噪音
  ai/
    AIManager.js       隊伍建立 & 交錯更新
    AIController.js    每士兵行為樹 + 戰鬥
    AIGrenade.js       手榴彈決策邏輯
    AIMovement.js      移動驗證 & 更新
    AIShooter.js       瞄準、散布、射擊
    AIVehicleController.js  載具登乘 & 駕駛 AI
    AIVisual.js        視覺更新、除錯渲染
  ...
  shared/
    CapsuleBody.js     膠囊碰撞輔助
    DamageFalloff.js   傷害衰減曲線
    DamageModel.js     集中式傷害計算
  systems/
    PhysicsWorld.js    cannon-es 封裝
    ScoreManager.js    旗點計分
    SpawnSystem.js     重生點選擇
    GrenadeManager.js  手榴彈生成 & 爆炸
    VehicleManager.js  載具生成 & 管理
    DroppedGunManager.js  掉落武器拾取系統
  vfx/
    TracerSystem.js    彈道曳光線
    ImpactVFX.js       命中粒子效果
  ui/
    HUDController.js   主 HUD 控制器
    Minimap.js         Canvas 小地圖
    KillFeed.js        擊殺通知
    SpectatorHUD.js    觀戰 UI 疊加層
  workers/
    navgrid-worker.js  背景執行緒 NavGrid 建構
    pathfind-worker.js 背景執行緒 A* 尋路
    threat-worker.js   背景執行緒威脅地圖計算
    threat-scan-worker.js  背景執行緒威脅掃描
```
